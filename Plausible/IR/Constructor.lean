...
def cbe_gen_block (cbe: backtrack_elem) : MetaM String := do
  let mut out := ""
...
def cbe_ifsome_block (cbe: backtrack_elem) : MetaM (String × String) := do
  let mut out := ""
  let iden := if cbe.gcc_group.ifsome_list.size > 0 then "  " else ""
  let mut is_ident:= false
  for gcc in cbe.gcc_group.ifsome_list do
    if is_ident then
      out := out  ++ iden ++ (← GenCheckCalls_toCode gcc) ++ " \n"
    else
      out := out  ++ (← GenCheckCalls_toCode gcc) ++ " \n"
      is_ident:= true
...
def cbe_gen_check_IR_block (cbe: backtrack_elem) (iden: String): MetaM (String × (List String)) := do
  let mut out := ""
  let mut vars : List String := []
  let mut checkcount := 1
  for gcc in cbe.gcc_group.check_IR_list do
    out := out ++ iden ++ "let check" ++ toString checkcount ++ " " ++ (← GenCheckCalls_toCode gcc) ++ " \n"
...
def cbe_return_checker (cbe: backtrack_elem) (iden: String) (vars: List String): MetaM String := do
  let mut out := ""
...
def backtrack_elem_toString_checker (cbe: backtrack_elem) : MetaM String := do
  let mut out := ""
  let matchblock ← cbe_match_block cbe
  let genblock ← cbe_gen_block cbe
  let (ifsomeblock, iden) ← cbe_ifsome_block cbe
  let (checkIRblock, vars) ← cbe_gen_check_IR_block cbe iden
  let returnblock ← cbe_return_checker cbe iden vars
  out := out ++ matchblock
...
/-
def backtrack_elem_toString_checker1 (cbe: backtrack_elem) : MetaM String := do
  let mut out := ""
  if cbe.mat_inp.size > 0 then
    out := out ++ "match "
    for i in cbe.mat_inp do
    ...
    out:= ⟨out.data.dropLast.dropLast⟩ ++ " =>  "
  for gcc in cbe.gcc_group.gen_list do
    out := out ++ " \n" ++ (← GenCheckCalls_toCode gcc)
  let iden := if cbe.gcc_group.ifsome_list.size > 0 then "  " else ""
  let mut is_ident:= false
  for gcc in cbe.gcc_group.ifsome_list do
    if is_ident then
      out := out ++ " \n" ++ iden ++ (← GenCheckCalls_toCode gcc)
    else
      out := out ++ " \n" ++ (← GenCheckCalls_toCode gcc)
      is_ident:= true
  let mut checkcount := 1
  for gcc in cbe.gcc_group.check_IR_list do
    out := out ++ " \n" ++ iden ++ "let check" ++ toString checkcount ++ " " ++ (← GenCheckCalls_toCode gcc)
    checkcount := checkcount + 1
  if cbe.var_eq.size + cbe.gcc_group.check_nonIR_list.size + cbe.gcc_group.check_IR_list.size > 0 then
    out := out ++ "\n" ++ iden ++ "return "
  else
    out := out ++ "return true"
  for j in [1:checkcount] do
    out := out ++ "check" ++ toString j ++ " && "
  for i in cbe.var_eq do
    out := out ++  "(" ++ toString (i.1.name) ++ " == " ++ toString (i.2.name) ++ ") && "
  for gcc in cbe.gcc_group.check_nonIR_list do
    out := out ++ (← GenCheckCalls_toCode gcc) ++ " && "
...
def checker_where_defs (relation: IR_info) (inpname: List String): MetaM String := do
  let mut out_str := ""
  let mut i := 0
  for con in relation.constructors do
    ...
    out_str:= out_str ++ (← prototype_for_checker_by_con relation inpname i) ++ ":= do \n"
    let bt ← get_checker_backtrack_elem_from_constructor con inpname
    let btStr ← backtrack_elem_toString_checker bt
...
def get_producer_backtrack_elem_from_constructor (con: IRConstructor) (inpname: List String) (genpos: Nat)
      : MetaM backtrack_elem := do
  let out_prop := con.out_prop
  let args := (out_prop.getAppArgs).toList
  let zipinp := inpname.zip args
  let zipinp := zipinp.take (genpos) ++ zipinp.drop (genpos+1)
  let need_match_zipimp :=  zipinp.filter (fun x => is_inductive_constructor x.2)
  let (mat_inp, mat_expr) := need_match_zipimp.unzip
  ...
  return {
    ...
    mat_expr := mat_expr.toArray
    gcc_group := gcc_group
    var_eq := gcc_group.var_eq
  }
-/


def cbe_if_return_producer (cbe: backtrack_elem) (iden: String) (vars: List String): MetaM String := do
  let mut out:= ""
  if cbe.var_eq.size + cbe.gcc_group.check_nonIR_list.size + cbe.gcc_group.check_IR_list.size > 0 then
    out := out ++ iden ++ "if "
  for j in vars do
...
def backtrack_elem_toString_producer (cbe: backtrack_elem) : MetaM String := do
  let mut out := ""
  let matchblock ← cbe_match_block cbe
  let genblock ← cbe_gen_block cbe
  let (ifsomeblock, iden) ← cbe_ifsome_block cbe
  let (checkIRblock, vars) ← cbe_gen_check_IR_block cbe iden
  let returnblock ← cbe_if_return_producer cbe iden vars
  out := out ++ matchblock
  if genblock.length > 0  ∧ out.length > 0 then
    out := out ++ "\n"
  out:= out ++ genblock
...
def producer_where_defs (relation: IR_info) (inpname: List String) (genpos: Nat): MetaM String := do
  ...
  for con in relation.constructors do
    ...
    out_str:= out_str ++ "\n-- Constructor: " ++ toString conprops_str
    out_str:= out_str ++ " → " ++ toString (← Meta.ppExpr con.out_prop) ++ "\n"
    out_str:= out_str ++ (← prototype_for_producer_by_con relation inpname genpos i) ++ ":= do\n"
    let bt ← get_producer_backtrack_elem_from_constructor con inpname genpos
    let btStr ← backtrack_elem_toString_producer bt
    out_str:= out_str ++ btStr ++ "\n"
  return out_str
