...
def mkArbitraryFueledInstance (targetTypeName : Name) : CommandElabM (TSyntax `command) := do
  ...
  let nonRecursiveGeneratorsWithoutWeights ←
  ...
  let mut caseExprs := #[]
  let zeroCase ← `(Term.matchAltExpr| | $(mkIdent ``Nat.zero) => $(mkIdent ``Gen.oneOfWithDefault) $defaultGenerator [$nonRecursiveGeneratorsWithoutWeights,*])
